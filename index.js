...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN: BRUXINHO MODS\n'//GANTI NAMA LU COK
            + 'ORG:CRIADOR DLCINHA;\n'//GANTI NAMA LU!!
            + 'TEL;type=CELL;type=VOICE;waid=19253296961:+1 925 329-6961\n'//GANTI NOMOR LU
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FadhilGraphy = ["19253296961@s.whatsapp.net"]
			...
			mess = {
				...
				only: {
					...
					ownerB: '*[⚠️]Desculpe, este comando só pode ser usado pelo proprietário !* ',
			...
			const ownerNumber = [
			"19253296961@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"19253296961@s.whatsapp.net",//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
		...
		case 'daftar':
			...
			case 'fadhil':
                ...
                setTimeout( () => {
					...
					setTimeout( () => {*/
					client.sendMessage(from, 'Salve,esta é minha lista de comandos :)', text, {quoted: mek})
					}, 0)
    				break
/*********PISAH MENU**********/
              case 'ownermenu':
...
/*********END PISAH MENU***********/
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				case 'owner':
				case 'creator':
                 client.sendMessage(from, {displayname: "+1 925 329-6961", vcard: vcard}, MessageType.contact, { quoted: mek})
         ...
         case 'lirik':
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('19253296961@s.whatsapp.net', options, text, {quoted: mek})
	   ...
	   case 'coli':
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 EL DLCINHA 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
